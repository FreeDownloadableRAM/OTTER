...
//activity
uniform sampler2D s_SecondaryDiffuse;
...
void main() {
	...
	float texSpec = texture(s_Specular, inUV).x; // added in by slides
	...
	vec4 textureColorPrimary = texture(s_Diffuse, inUV); // added in by slides

	//activity
	vec4 textureColorSecondary = texture(s_SecondaryDiffuse, inUV); // for second texture

	vec4 textureColor = mix(textureColorPrimary, textureColorSecondary, u_TextureMixing);
