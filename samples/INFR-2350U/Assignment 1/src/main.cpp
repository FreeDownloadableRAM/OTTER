...
bool initGLFW() {
	...
	window = glfwCreateWindow(800, 800, "Assignment 1 Basics: Hossain Alinaqi 100761694", nullptr, nullptr);
...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader->LoadShaderPartFromFile("shaders/vertex_terrain.glsl", GL_VERTEX_SHADER);
		...
		glm::vec3 heightCutoffs = glm::vec3(0.05f, 0.35f, 0.60f);
		glm::vec3 interpolationFactors = glm::vec3(0.04f, 0.08f, 0.08f);
		float     transparency = 0.25f;
		//togglers 1 = T, 0 = F

		float     lightingToggle = 1.0f;
		float     ambientOnlyToggle = 0.0f;
		float     specularOnlyToggle = 0.0f;
		//float     waveTime = 0.0f;
		...
		shader->SetUniform("u_InterpolateFactors", interpolationFactors);
		shader->SetUniform("u_Transparency", transparency);
		//shader->SetUniform("time", waveTime);
		//togglers
		shader->SetUniform("u_LightingToggle", lightingToggle);
		shader->SetUniform("u_AmbientOnlyToggle", ambientOnlyToggle);
		shader->SetUniform("u_SpecularOnlyToggle", specularOnlyToggle);
		...
		Texture2D::sptr diffuse = Texture2D::LoadFromFile("images/water.jpg");
		Texture2D::sptr diffuse2 = Texture2D::LoadFromFile("images/water.jpg");
		...
		material0->Set("u_Transparency", 1.0f);
		
		material0->Set("u_LightingToggle", 1.0f); // 1 = true // 0 = false
		material0->Set("u_AmbientOnlyToggle", 0.0f); // 1 = true // 0 = false
		material0->Set("u_SpecularOnlyToggle", 0.0f); // 1 = true // 0 = false
		...
		reflective->LoadShaderPartFromFile("shaders/frag_blinn_phong_textured.glsl", GL_FRAGMENT_SHADER);
		reflective->Link();
		
		//Shader::sptr transparent = Shader::Create();
		//reflective->LoadShaderPartFromFile("shaders/vertex_shader.glsl", GL_VERTEX_SHADER);
		//reflective->LoadShaderPartFromFile("shaders/frag_terrain.glsl", GL_FRAGMENT_SHADER);
		//reflective->Link();
		...
		material1->Set("u_Transparency", 1.0f);

		//material1->Set("u_LightingToggle", 1.0f); // 1 = true // 0 = false
		...
		reflectiveMat->Set("u_Transparency", 0.25f);


		
		//ShaderMaterial::sptr transparentMat = ShaderMaterial::Create();
		//transparentMat->Shader = transparent;
		//transparentMat->Set("s_Environment", environmentMap);
		//transparentMat->Set("u_EnvironmentRotation", glm::mat3(glm::rotate(glm::mat4(1.0f), glm::radians(90.0f), glm::vec3(1, 0, 0))));
		//transparentMat->Set("u_Transparency", 0.15f);
		...
		GameObject sceneObj = scene->CreateEntity("scene_geo"); 
		{
			...
			sceneObj.get<Transform>().SetLocalPosition(110.0f, 0.0f, 0.0f);
		}
		...
		GameObject obj2 = scene->CreateEntity("monkey_quads");
		{
			...
			obj2.get<Transform>().SetLocalPosition(4.75f, 3.25f, 1.0f);
		...
		GameObject obj3 = scene->CreateEntity("monkey_tris");
		{
			...
			obj3.get<Transform>().SetLocalPosition(0.0f, 0.0f, -0.5f).SetLocalScale(glm::vec3(7, 7, 7));
		...
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/terrain_triangulated.obj");
			
			obj5.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material0); 
			// x = mountain width left to right, y = mountain height? , z length up and down
			obj5.get<Transform>().SetLocalPosition(0.0f, 0.0f, -0.1f).SetLocalScale(glm::vec3(1,2,1));
		...
		GameObject obj4 = scene->CreateEntity("moving_box");
		{
			...
			obj4.get<Transform>().SetLocalPosition(12.0f, 0.0f, 1.0f);
			...
			pathing->Points.push_back({ 1.95f,  -5.5f, -0.5f });
			pathing->Points.push_back({ 1.93f,  -5.5f, -10.5f });
			pathing->Points.push_back({ 1.90f,  -5.5f, -5.5f });
			pathing->Points.push_back({ 1.97f,  -5.5f, -0.4f });
			pathing->Speed = 2.0f;
		}
		/*
		GameObject obj6 = scene->CreateEntity("following_monkey");
		{
			...
			obj6.get<Transform>().SetLocalPosition(10.0f, 0.0f, 3.0f);
		...
		*/
		...
		while (!glfwWindowShouldClose(window)) {
			...
			if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS) {
				...
				lightingToggle = 0.0f;
				cout << "light Toggle Is: " << lightingToggle << "\n";
			...
			if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) {
				...
				ambientOnlyToggle = 1.0f;
				cout << "ambientOnly Toggle Is: " << ambientOnlyToggle << "\n";

			}
			if (glfwGetKey(window, GLFW_KEY_X) == GLFW_PRESS) {


				ambientOnlyToggle = 0.0f;
				cout << "ambientOnly Toggle Is: " << ambientOnlyToggle << "\n";
			...
			if (glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS) {
				...
				specularOnlyToggle = 1.0f;
				cout << "specularOnly Toggle Is: " << specularOnlyToggle << "\n";

			}
			if (glfwGetKey(window, GLFW_KEY_C) == GLFW_PRESS) {


				specularOnlyToggle = 0.0f;
				cout << "specularOnly Toggle Is: " << specularOnlyToggle << "\n";
