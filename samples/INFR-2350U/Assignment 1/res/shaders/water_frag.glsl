...
//ILE 5
in vec4 gl_FragCoord; //get screen space
...
void main() {
	...
	vec2 screenUV = vec2(gl_FragCoord.x / u_WindowWidth, gl_FragCoord.y / u_WindowHeight); // get screen coords / pixels
	float expectedDepth = linearize_depth(texture(s_DepthBuffer, screenUV).x, 0.01, 1000.0);
	...
	vec4 textureColor = mix(textureColor1, textureColor2, u_TextureMix);
		...
		) * inColor * textureColor.rgb; //obj color
	...
	*/
	...
	//frag_color = vec4(expectedDepth, expectedDepth, expectedDepth, 1.0);
