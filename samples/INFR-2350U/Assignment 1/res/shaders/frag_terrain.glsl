...
uniform float u_LightingToggle; // 1 = T (ON), 0 = F (OFF)
uniform float u_AmbientOnlyToggle; // 1 = T (ON), 0 = F (OFF)
uniform float u_SpecularOnlyToggle; // 1 = T (ON), 0 = F (OFF)
...
void main() {
	...
	if (u_LightingToggle == 1.0f){ //True
	
	}
	else{ //lighting is Off
	
	vec3 u_AmbientCol = u_AmbientCol * 0;
	float u_AmbientStrength = 0.0;

	}
	
	//ambient only?

	if (u_AmbientOnlyToggle == 1.0f){ //True
	
	vec3 diffuse =  diffuse * 0;
	vec3 specular = specular * 0;

	}
	else{ //ambient only off
	...
	if (u_SpecularOnlyToggle == 1.0f){ //True
	...
	vec3 result = (
		...
		) * inColor * textureColor.rgb * u_Transparency; // Object color

	frag_color = vec4(result, textureColor.a);
	
	
	//frag_color = inTexWeights;
}